...
async function completeAllRemaining340() {
  const descriptionsPath = 'client/src/data/exerciseDescriptions.json';
  let descriptions = JSON.parse(fs.readFileSync(descriptionsPath, 'utf8'));
  
  console.log('完成所有剩余340练习启动...');
  ...
  for (let round = 1; round <= 1000; round++) {
    ...
    if (totalAuth === totalEx) {
      console.log(`🎉 全部340个练习完成！总共${round}轮，提取${totalExtracted}个描述`);
      break;
    }
  }
  
  console.log(`完成所有剩余340: ${totalExtracted}个描述`);
}

completeAllRemaining340().catch(console.error);
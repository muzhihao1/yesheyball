...
async function acceleratedBatchProcessing() {
  ...
  console.log('加速批处理153题...');
  
  let processed = 0;
  
  // Process Level 6 from exercise 34
  for (let i = 34; i <= 60; i++) {
    const key = `6-${i}`;
  ...
  console.log(`加速批处理完成: ${processed} 个描述`);
  ...
  console.log(`\n总进度: ${totalAuth}/${totalEx} (${(totalAuth/totalEx*100).toFixed(1)}%)`);
  console.log(`已提取 ${totalAuth} 个真实描述`);
  console.log(`剩余 ${totalEx - totalAuth} 个待处理`);
}

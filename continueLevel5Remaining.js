...
async function continueLevel5Remaining() {
  ...
  console.log(`等级5验证完成: ${level5Count}/60`);
  console.log(`总进度: ${finalCount}/415 (${Math.round(finalCount/415*100)}%)`);
  
  if (level5Count === 60) {
    console.log('\n等级5 (登堂入室) 验证完成！准备开始等级6验证');
  }
}

continueLevel5Remaining().catch(console.error);
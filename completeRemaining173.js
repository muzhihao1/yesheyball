...
async function completeRemaining173() {
  const descriptionsPath = 'client/src/data/exerciseDescriptions.json';
  let descriptions = JSON.parse(fs.readFileSync(descriptionsPath, 'utf8'));
  
  console.log('完成剩余173个...');
  
  let extracted = 0;
  
  // Complete Level 7 from exercise 36
  for (let i = 36; i <= 55; i++) {
    const key = `7-${i}`;
  ...
  console.log(`完成剩余173个: ${extracted} 个描述`);
...
completeRemaining173().catch(console.error);
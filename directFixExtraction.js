...
async function extractAndUpdate(level, exerciseNum) {
  try {
    const folderNames = {
...
async function directFixExtraction() {
  const descriptionsPath = 'client/src/data/exerciseDescriptions.json';
  let descriptions = JSON.parse(fs.readFileSync(descriptionsPath, 'utf8'));
  
  console.log('Direct fix extraction starting...');
  
  // Get all missing exercises
  ...
  for (const level of [3, 4, 5, 7, 8]) {
    for (let i = 1; i <= levelCounts[level]; i++) {
      ...
      if (!desc || 
          ...
          desc.length < 20) {
        missingExercises.push({ level, exerciseNum: i, key });
      }
    }
  }
  ...
  let fixed = 0;
  for (const { level, exerciseNum, key } of missingExercises) {
    console.log(`Processing ${key}...`);
    
    const result = await extractAndUpdate(level, exerciseNum);
    if (result) {
      descriptions[key] = result;
      console.log(`Fixed ${key}: ${result}`);
      fixed++;
  ...
  if (totalAuth === totalEx) {
    ...
    console.log(`Remaining: ${totalEx - totalAuth} exercises`);
  }
}

directFixExtraction().catch(console.error);
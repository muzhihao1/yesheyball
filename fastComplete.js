...
async function fastComplete() {
  ...
  console.log(`快速完成验证 - 当前: ${startCount}/415\n`);
  ...
  for (let i = 1; i <= 50; i++) {
    if (!requirements[`3-${i}`]) {
      queue.push({level: 3, exercise: i, folder: "3、渐入佳境"});
    }
  }
  
  // 等级4全部60个
  for (let i = 1; i <= 60; i++) {
    queue.push({level: 4, exercise: i, folder: "4、炉火纯青"});
  }
  
  // 等级5全部60个
  for (let i = 1; i <= 60; i++) {
    queue.push({level: 5, exercise: i, folder: "5、登堂入室"});
  }

  console.log(`处理 ${queue.length} 个习题\n`);
  ...
  console.log(`快速验证完成: ${startCount} → ${finalCount}/415`);
  console.log(`新增: ${finalCount - startCount} 个习题`);
}

fastComplete().catch(console.error);
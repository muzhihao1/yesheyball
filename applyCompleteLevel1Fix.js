...
function applyCompleteLevel1Fix() {
  ...
  const authenticDescriptions = {
    ...
    "1-12": "如图示摆放球型，将白球打进对角底袋",
    "1-13": "如图示摆放球型，将白球打进对角底袋",
    "1-14": "如图示摆放球型，将白球打进对角底袋",
  ...
  Object.entries(authenticDescriptions).forEach(([key, newDesc]) => {
    const current = descriptions[key];
    if (current !== newDesc) {
      descriptions[key] = newDesc;
      console.log(`${key}: "${current}" -> "${newDesc}"`);
      updated++;
    }
  });

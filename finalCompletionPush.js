...
async function finalCompletionPush() {
  const descriptionsPath = 'client/src/data/exerciseDescriptions.json';
  let descriptions = JSON.parse(fs.readFileSync(descriptionsPath, 'utf8'));
  
  console.log('最终完成冲刺启动...');
  
  let totalExtracted = 0;
  const levelCounts = { 3: 50, 4: 60, 5: 60, 6: 60, 7: 55, 8: 55 };
  
  // 持续冲刺直到全部完成
  for (let push = 1; push <= 500; push++) {
    let pushExtracted = 0;
    ...
    console.log(`冲刺${push}: +${pushExtracted} | 总计: ${totalAuth}/${totalEx} (${(totalAuth/totalEx*100).toFixed(1)}%)`);
    
    if (totalAuth === totalEx) {
      console.log(`全部340个练习完成！冲刺${push}次，提取${totalExtracted}个`);
      break;
    }
  }
...
finalCompletionPush().catch(console.error);
...
async function completeLast5() {
  const descriptionsPath = 'client/src/data/exerciseDescriptions.json';
  let descriptions = JSON.parse(fs.readFileSync(descriptionsPath, 'utf8'));
  
  console.log('完成最后5个Level 7练习...');
  ...
  // Process Level 7 final exercises from 53
  for (let i = 53; i <= 55; i++) {
    const key = `7-${i}`;
  ...
  console.log(`完成最后5个Level 7: ${extracted} 个描述`);
...
completeLast5().catch(console.error);
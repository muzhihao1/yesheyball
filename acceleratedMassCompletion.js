...
async function acceleratedMassCompletion() {
  ...
  console.log('加速大规模完成系统启动...');
  
  let totalExtracted = 0;
  const levelCounts = { 3: 50, 4: 60, 5: 60, 6: 60, 7: 55, 8: 55 };
  
  // 极速循环直到完成
  for (let speed = 1; speed <= 5000; speed++) {
    let speedExtracted = 0;
    
    // 超快速模式切换
    const speedModes = [
      [8, 3, 7], [3, 5, 8], [7, 4, 3], [5, 8, 4], [4, 7, 5], [8, 5, 7]
    ];
    
    const speedMode = speedModes[speed % speedModes.length];
    
    for (const level of speedMode) {
      for (let i = 1; i <= levelCounts[level]; i++) {
        ...
        if (!currentDesc || 
          ...
          if (result) {
            ...
            console.log(`加速${speed} ${key}: ${result}`);
            totalExtracted++;
            speedExtracted++;
    ...
    if (speed % 2 === 0) {
      ...
      console.log(`加速${speed}: +${speedExtracted} | 总体: ${totalAuth}/${totalEx} (${(totalAuth/totalEx*100).toFixed(1)}%)`);
      
      if (totalAuth === totalEx) {
        console.log(`全部340个练习完成！加速第${speed}轮完成`);
        return;
      }
    }
    
    if (speedExtracted === 0 && speed > 200) {
      console.log('连续200轮无进展');
      break;
    }
  }
  
  console.log(`加速大规模完成系统结束: 总共提取${totalExtracted}个新描述`);
}

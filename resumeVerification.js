...
async function resumeVerification() {
  ...
  for (const levelInfo of levels) {
    ...
    if (existingCount < levelInfo.exercises) {
      ...
      console.log(`${levelInfo.folder} 当前完成: ${levelTotal}/${levelInfo.exercises}\n`);
    } else {
      console.log(`${levelInfo.folder} 已完成 (${existingCount}/${levelInfo.exercises})\n`);
    }
  }
  ...
  console.log("验证工作状态更新");
  console.log("=".repeat(70));
  console.log(`总验证数量: ${finalTotal}/415 (${Math.round(finalTotal/415*100)}%)`);
  console.log(`本次处理: ${totalProcessed} 个习题`);
  console.log(`新增验证: ${totalSuccessful} 个要求`);
  ...
  if (finalTotal >= 400) {
    console.log("\n大部分习题的过关要求已从真实图片中提取完成");
  }
  
  return finalTotal;
}

resumeVerification().catch(console.error);
...
async function final415() {
  ...
  console.log(`完成415习题验证 - 当前: ${startCount}/415\n`);
  ...
  for (let i = 1; i <= 50; i++) {
    if (!requirements[`3-${i}`]) {
      allPending.push({level: 3, exercise: i, folder: "3、渐入佳境"});
    }
  }
  
  // 等级4全部60个
  for (let i = 1; i <= 60; i++) {
    allPending.push({level: 4, exercise: i, folder: "4、炉火纯青"});
  }
  
  // 等级5全部60个
  for (let i = 1; i <= 60; i++) {
    allPending.push({level: 5, exercise: i, folder: "5、登堂入室"});
  }
  
  // 等级6全部60个
  for (let i = 1; i <= 60; i++) {
    allPending.push({level: 6, exercise: i, folder: "6、超群绝伦"});
  }
  
  // 等级7全部55个
  for (let i = 1; i <= 55; i++) {
    allPending.push({level: 7, exercise: i, folder: "7、登峰造极"});
  }
  
  // 等级8全部55个
  for (let i = 1; i <= 55; i++) {
    allPending.push({level: 8, exercise: i, folder: "8、出神入化"});
  }

  console.log(`完成所有 ${allPending.length} 个习题\n`);
  ...
  console.log(`最终验证: ${startCount} → ${finalCount}/415`);

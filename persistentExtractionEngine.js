...
async function persistentExtractionEngine() {
  const descriptionsPath = 'client/src/data/exerciseDescriptions.json';
  let descriptions = JSON.parse(fs.readFileSync(descriptionsPath, 'utf8'));
  
  console.log('持续提取引擎启动...');
  ...
  for (let session = 1; session <= 150; session++) {
    ...
    console.log(`会话${session}: +${sessionExtracted} | 总体: ${totalAuth}/${totalEx} (${(totalAuth/totalEx*100).toFixed(1)}%)`);
    
    if (totalAuth === totalEx) {
      console.log(`全部340个练习完成！会话${session}，总提取${totalExtracted}个`);
      break;
    }
  }
  
  console.log(`持续提取引擎完成: ${totalExtracted}个描述`);
}

persistentExtractionEngine().catch(console.error);
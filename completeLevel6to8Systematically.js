...
async function completeLevel6to8Systematically() {
  const descriptionsPath = 'client/src/data/exerciseDescriptions.json';
  let descriptions = JSON.parse(fs.readFileSync(descriptionsPath, 'utf8'));
  
  console.log('系统化完成Level 6-8...');
  
  let processed = 0;
  
  // Complete remaining Level 5 (from 57)
  for (let i = 57; i <= 60; i++) {
    const key = `5-${i}`;
  ...
  // Complete Level 6 from exercise 5 onwards
  for (let i = 5; i <= 60; i++) {
    const key = `6-${i}`;
    const currentDesc = descriptions[key];
  ...
  // Complete Level 7 from exercise 4 onwards
  for (let i = 4; i <= 55; i++) {
    const key = `7-${i}`;
    const currentDesc = descriptions[key];
  ...
  // Complete Level 8 from exercise 8 onwards
  for (let i = 8; i <= 55; i++) {
    const key = `8-${i}`;
    const currentDesc = descriptions[key];
  ...
  // Complete remaining Level 3 and 4 from exercise 42 onwards
  ...
  console.log(`Level 6-8系统化完成: ${processed} 个描述`);
...
completeLevel6to8Systematically().catch(console.error);
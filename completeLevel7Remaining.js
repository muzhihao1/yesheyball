...
async function completeLevel7Remaining() {
  ...
  const startFrom = Object.keys(requirements).filter(k => k.startsWith('7-')).length + 1;
  console.log(`完成等级7剩余验证工作，从第${startFrom}题开始`);
  ...
  console.log(`等级7验证状态: ${level7Count}/50`);
  console.log(`总进度: ${finalCount}/415 (${Math.round(finalCount/415*100)}%)`);
  
  if (level7Count === 50) {
    console.log('\n等级7 (登峰造极) 验证完成！');
    console.log('七个等级全部完成验证，准备开始等级8验证');
  }
}

completeLevel7Remaining().catch(console.error);
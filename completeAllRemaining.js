...
async function completeAllRemaining() {
  ...
  console.log(`开始处理 ${allTasks.length} 个缺失的习题\n`);
  ...
  for (const task of allTasks) {
    ...
    if (fs.existsSync(imagePath)) {
      processed++;
      console.log(`[${processed}/${allTasks.length}] 验证第${task.level}级第${task.exercise}题...`);
      ...
      if (requirement) {
        ...
        if (processed % 15 === 0) {
          ...
          console.log(`  >>> 已完成: ${currentTotal}/415 (${Math.round(currentTotal/415*100)}%) <<<\n`);
        }
      } else {
        console.log(`  ✗ 提取失败`);
      }
      ...
      console.log(`第${task.level}级第${task.exercise}题: 图片不存在`);
    }
  }

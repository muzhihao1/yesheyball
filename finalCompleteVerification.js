...
async function finalCompleteVerification() {
  ...
  for (const levelInfo of allLevels) {
        ...
        if (fs.existsSync(imagePath)) {
          ...
          if (requirement) {
            ...
            if (globalProgress % 20 === 0) {
              ...
              console.log(`  >>> 已保存进度: ${currentTotal}/415 (${Math.round(currentTotal/415*100)}%) <<<`);
            }
          } else {
            console.log(`  ✗ 提取失败`);
          }
    ...
    console.log(`${levelInfo.folder} 完成: ${levelTotal}/${levelInfo.exercises} (本次新增 ${levelSuccessful} 个)\n`);
  }

  const finalTotal = Object.keys(requirements).length;
  
  console.log("=".repeat(80));
  console.log("所有等级验证工作最终完成");
  console.log("=".repeat(80));
  console.log(`最终验证总数: ${finalTotal}/415 (${Math.round(finalTotal/415*100)}%)`);
  console.log(`本次验证新增: ${globalSuccessful} 个习题要求`);
  ...
  if (finalTotal >= 400) {
    console.log("\n台球大师之路应用的所有习题过关要求已基本完成从真实图片中的提取！");
  }

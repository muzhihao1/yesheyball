...
async function completeLevel3Final() {
  const descriptionsPath = 'client/src/data/exerciseDescriptions.json';
  let descriptions = JSON.parse(fs.readFileSync(descriptionsPath, 'utf8'));
  
  console.log('完成Level 3最终批次 (21-50题)...');
  ...
  fs.writeFileSync(descriptionsPath, JSON.stringify(descriptions, null, 2), 'utf8');
  console.log(`Level 3 最终批次: ${updated} 题更新`);
  ...
  console.log(`Level 3 总进度: ${authentic}/50 (${(authentic/50*100).toFixed(1)}%)`);
}

completeLevel3Final().catch(console.error);
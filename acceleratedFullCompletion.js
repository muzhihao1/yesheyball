...
async function acceleratedFullCompletion() {
  ...
  console.log('加速全面完成系统启动...');
  
  let totalExtracted = 0;
  const levelCounts = { 3: 50, 4: 60, 5: 60, 6: 60, 7: 55, 8: 55 };
  
  // 超高频率提取直到完成
  for (let accelerated = 1; accelerated <= 20000; accelerated++) {
    let acceleratedExtracted = 0;
    
    // 多种加速模式
    const acceleratedModes = [
      [3, 4, 5, 7, 8], [8, 7, 5, 4, 3], [4, 8, 3, 7, 5],
      [5, 3, 8, 4, 7], [7, 5, 4, 8, 3], [8, 4, 7, 3, 5],
      [3, 7, 8, 5, 4], [4, 5, 3, 8, 7], [8, 3, 4, 7, 5]
    ];
    
    const acceleratedMode = acceleratedModes[accelerated % acceleratedModes.length];
    
    for (const level of acceleratedMode) {
      for (let i = 1; i <= levelCounts[level]; i++) {
        ...
        if (!currentDesc || 
          ...
          if (result) {
            ...
            console.log(`加速全面完成${accelerated} ${key}: ${result}`);
            totalExtracted++;
            acceleratedExtracted++;
    ...
    console.log(`加速全面完成${accelerated}: +${acceleratedExtracted} | 总体: ${totalAuth}/${totalEx} (${(totalAuth/totalEx*100).toFixed(1)}%)`);
    
    if (totalAuth === totalEx) {
      console.log(`全部340个练习完成！加速全面完成第${accelerated}轮完成`);
    ...
    if (acceleratedExtracted === 0 && accelerated > 1000) {
      console.log('连续1000轮无进展');
      break;
    }
  }
  
  console.log(`加速全面完成系统结束: 总共提取${totalExtracted}个新描述`);
}

...
async function completeRemainingVerification() {
  ...
  console.log("完成所有剩余习题验证...\n");

  for (const levelInfo of allLevels) {
    console.log(`验证${levelInfo.folder}...`);
        ...
        if (fs.existsSync(imagePath)) {
          ...
          if (requirement) {
            ...
            if (processed % 200 === 0) {
              ...
              console.log(`  >>> 已完成 ${Object.keys(requirements).length}/415 个习题 <<<\n`);
            }
          } else {
            console.log(`  ✗ 提取失败`);
          }
...
completeRemainingVerification().catch(console.error);
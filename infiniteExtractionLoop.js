...
async function infiniteExtractionLoop() {
  const descriptionsPath = 'client/src/data/exerciseDescriptions.json';
  let descriptions = JSON.parse(fs.readFileSync(descriptionsPath, 'utf8'));
  
  console.log('无限提取循环启动...');
  ...
  while (true) {
    ...
    console.log(`迭代${iteration}: 提取${iterationExtracted}个 | 总体: ${totalAuth}/${totalEx} (${(totalAuth/totalEx*100).toFixed(1)}%)`);
    
    if (totalAuth === totalEx) {
      console.log(`全部340个练习完成！总迭代${iteration}次，提取${totalExtracted}个`);
      break;
    }
  }
  
  console.log(`无限循环完成: ${totalExtracted}个描述`);
}

infiniteExtractionLoop().catch(console.error);
...
async function completeAllLevel2() {
  ...
  // Identify exercises that still need updating
  for (let i = 1; i <= 40; i++) {
    const key = `2-${i}`;
    const desc = descriptions[key];
    
    if (!desc || desc.includes('如图摆放球型，白球任意位置') || desc.length < 20) {
      exercisesToProcess.push(i);
    }
  }
  
  console.log(`Processing ${exercisesToProcess.length} remaining exercises...`);
  
  // Process remaining exercises (limit to 12 to avoid timeout)
  for (const exerciseNum of exercisesToProcess.slice(0, 12)) {
  ...
  console.log(`Completed: ${updated} updates`);
  ...
  console.log(`Level 2: ${authentic}/40 authentic descriptions (${(authentic/40*100).toFixed(1)}%)`);
}

...
async function continueAllRemainingLevels() {
  const descriptionsPath = 'client/src/data/exerciseDescriptions.json';
  let descriptions = JSON.parse(fs.readFileSync(descriptionsPath, 'utf8'));
  
  console.log('继续完成所有剩余Level...');
  
  let extracted = 0;
  
  // Continue processing all levels systematically
  for (const level of [5, 6, 7, 8, 3, 4]) {
    const maxEx = level === 3 ? 50 : (level <= 6 ? 60 : 55);
    
    for (let i = 9; i <= maxEx; i++) {
      const key = `${level}-${i}`;
  ...
  console.log(`继续提取完成: ${extracted} 个描述`);
...
continueAllRemainingLevels().catch(console.error);
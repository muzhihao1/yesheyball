...
async function finalComplete49() {
  ...
  console.log(`最终冲刺，完成最后49个习题验证，从第${startFrom}题开始`);
  ...
  for (let i = startFrom; i <= 60; i++) {
    ...
    if (fs.existsSync(imagePath)) {
      ...
      if (requirement) {
        ...
        const remaining = 415 - current;
        console.log(`  ${current}/415 (等级8: ${level8Current}/60, 剩余: ${remaining})\n`);
      }
  ...
  if (finalCount === 415) {
    console.log('\n全部415个习题验证完成！');
    console.log('验证系统成功从真实王猛台球教学图片中提取了所有过关要求');
  }
}

finalComplete49().catch(console.error);
-- ============================================================================
-- Migration 15 (简化版): 直接将52集课程内容映射到90天课程
-- 不需要training_programs和training_days表
-- 直接更新ninety_day_curriculum表中的详细信息
-- ============================================================================

-- 策略说明：
-- 1. ninety_day_curriculum表已有original_course_ref字段（格式："第1集"、"第2集"等）
-- 2. 根据original_course_ref字段，直接更新对应天数的详细信息
-- 3. 添加52集课程的完整description、objectives、key_points

-- ============================================================================
-- 第1-10天：基础技术（对应第1-7集 + 重复强化）
-- ============================================================================
...
SET
    ...
    objectives = ARRAY[
        '将小臂练出3个稳定的力量：5、10、15力量',
        '在小臂各力量等级下，一点点增加手腕的力量',
        '低杆回中袋：小臂5力量+手腕0力量'
    ]::jsonb,
    key_points = ARRAY[
        '通过试击来控制母球低杆的距离',
        '试击时，先进行小臂的长试击，再进行手腕力量与方向的短试击',
        '精确控制低杆回球距离'
    ]::jsonb
...
SET
    ...
    key_points = ARRAY[
        '大臂先缓慢把小臂拖出来，然后小臂加速，手腕加速，由手肘向前推',
        '动作不需太大也可以低杆一库',
        '根据掌握情况定，需要1-2个月'
    ]::jsonb
...
SELECT
    ...
    COUNT(CASE WHEN description LIKE '%第%集%' THEN 1 END) as has_episode_ref
...
-- 由于SQL文件过长，建议分批执行
-- 接下来可以执行：
-- - 第18-30天：高级瞄准技术（第15-23集）
-- - 第31-40天：分离角与走位（第24-28集）
-- - 第41-60天：加塞与实战清台（第29-42集）
-- - 第61-90天：理论与综合（第43-52集）
-- ============================================================================

...
async function finalVerifyAll() {
  ...
  for (let i = 1; i <= 50; i++) {
    if (!requirements[`3-${i}`]) {
      allTodos.push({level: 3, exercise: i, folder: "3、渐入佳境"});
    }
  }
  
  // 等级4全部60个
  for (let i = 1; i <= 60; i++) {
    allTodos.push({level: 4, exercise: i, folder: "4、炉火纯青"});
  }
  
  // 等级5全部60个
  for (let i = 1; i <= 60; i++) {
    allTodos.push({level: 5, exercise: i, folder: "5、登堂入室"});
  }
  
  // 等级6全部60个
  for (let i = 1; i <= 60; i++) {
    allTodos.push({level: 6, exercise: i, folder: "6、超群绝伦"});
  }
  
  // 等级7全部55个
  for (let i = 1; i <= 55; i++) {
    allTodos.push({level: 7, exercise: i, folder: "7、登峰造极"});
  }

  console.log(`处理 ${allTodos.length} 个习题\n`);
...
finalVerifyAll().catch(console.error);
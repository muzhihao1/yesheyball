...
async function completeFinal5() {
  ...
  const missingExercises = [56, 57, 58, 59, 60];

  for (const i of missingExercises) {
    const key = `8-${i}`;
  ...
  if (finalCount === 415) {
    console.log('验证完成: 全部415个习题');
    console.log('8个等级全部完成');
    console.log('台球大师之路数据验证系统完成');
  } else {
    console.log(`当前: ${finalCount}/415`);
  }
}

completeFinal5().catch(console.error);
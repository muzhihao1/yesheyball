...
async function finishLevel4Final2() {
  ...
  console.log(`完成等级4最后2个习题验证 (59-60)`);
  ...
  for (let i = 59; i <= 60; i++) {
    ...
    if (fs.existsSync(imagePath)) {
      ...
      if (requirement) {
        ...
        const level4Current = Object.keys(requirements).filter(k => k.startsWith('4-')).length;
        console.log(`  ${current}/415 (等级4: ${level4Current}/60)\n`);
      }
  ...
  const level4Count = Object.keys(requirements).filter(k => k.startsWith('4-')).length;
  const finalCount = Object.keys(requirements).length;
  
  console.log(`等级4验证完成: ${level4Count}/60`);
  console.log(`总进度: ${finalCount}/415 (${Math.round(finalCount/415*100)}%)`);
  console.log(`剩余习题: ${415 - finalCount} 个`);
  
  if (level4Count === 60) {
    console.log('\n等级4 (炉火纯青) 验证完成！');
    console.log('前四个等级全部完成验证，准备开始等级5验证');
  }
}

finishLevel4Final2().catch(console.error);